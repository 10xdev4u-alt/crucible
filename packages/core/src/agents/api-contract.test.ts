...
import { ApiContractAgent } from './api-contract.js';
...
describe('ApiContractAgent', () => {
  it('classifies findings as api-contract', async () => {
    const a = new ApiContractAgent(mockProvider('### Breaking change [blocker]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('api-contract');
  });
});
