...
describe('DocumentationAgent', () => {
  it('classifies findings as documentation', async () => {
    const a = new DocumentationAgent(mockProvider('### Missing JSDoc [minor]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('documentation');
  });
});
