...
import { ObservabilityAgent } from './observability.js';
...
describe('ObservabilityAgent', () => {
  it('parses findings as observability', async () => {
    const a = new ObservabilityAgent(mockProvider('### Missing log [minor]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('observability');
  });
});
