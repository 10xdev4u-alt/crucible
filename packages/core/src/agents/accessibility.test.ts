...
describe('AccessibilityAgent', () => {
  it('classifies findings as accessibility', async () => {
    const a = new AccessibilityAgent(mockProvider('### Missing alt [major]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('accessibility');
  });
});
