...
describe('ArchitectureAgent', () => {
  it('classifies findings as architecture', async () => {
    const a = new ArchitectureAgent(mockProvider('### Tight coupling [major]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('architecture');
  });
});
