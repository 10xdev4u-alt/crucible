...
import { ErrorHandlingAgent } from './error-handling.js';
...
describe('ErrorHandlingAgent', () => {
  it('parses findings as error-handling', async () => {
    const a = new ErrorHandlingAgent(mockProvider('### Swallowed error [major]\n**Message:** x'));
    ...
    expect(out.findings[0]?.category).toBe('error-handling');
  });
});
