...
import { DataIntegrityAgent } from './data-integrity.js';
...
describe('DataIntegrityAgent', () => {
  it('classifies findings as data-integrity', async () => {
    const a = new DataIntegrityAgent(
      mockProvider('### Missing validation [major]\n**Message:** x'),
    );
    ...
    expect(out.findings[0]?.category).toBe('data-integrity');
  });
});
